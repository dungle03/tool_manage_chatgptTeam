...
describe("MemberTable over-limit classification", () => {
  it("no highlight when active members <= 5", () => {
    const members = [
  ...
  it("highlights exactly 6th+ oldest active members", () => {
    ...
    expect(screen.getByText("member6@team.test").closest("tr")).toHaveClass("member-overlimit-row");
    expect(screen.getByText("member7@team.test").closest("tr")).toHaveClass("member-overlimit-row");

    expect(screen.getByText("member1@team.test").closest("tr")).not.toHaveClass("member-overlimit-row");
    expect(screen.getByText("member5@team.test").closest("tr")).not.toHaveClass("member-overlimit-row");
  });
