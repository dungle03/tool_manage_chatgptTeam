...
def import_team(
    ...
    if not access_token and not session_token:
        raise HTTPException(status_code=400, detail="access_token or session_token is required")

    if not access_token and session_token:
        try:
            refreshed = asyncio.run(chatgpt_service.refresh_access_token(session_token, payload.org_id))
    ...
    if not accounts:
        raise HTTPException(status_code=404, detail="no team account found for provided token")
    ...
    for info in accounts:
        ...
        if existing:
            ...
            existing.member_limit = int(info.get("member_limit") or existing.member_limit or 7)
    ...
    for index, item in enumerate(remote_invites, start=1):
        ...
        session.add(
            Invite(
                ...
                invite_id=str(item.get("id") or item.get("invite_id") or f"inv_{workspace.org_id}_{index}"),
